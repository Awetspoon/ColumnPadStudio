...
public sealed class MainViewModel : NotifyBase
{
    ...
    public void RefreshStatus()
    {
        ...
        StatusText = $"Columns: {Columns.Count}    Selected: {active?.Title ?? "-"}    Line nums: {(ShowLineNumbers ? "On" : "Off")}    Wrap: {(WordWrap ? "On" : "Off")}    Font: {EditorFontFamily} {EditorFontStyleName} {EditorFontSize:0}    Theme: {ThemePreset}{checkText}";
    }
    ...
    private void SetCurrentFileReference(string? path, SaveFileKind kind)
    {
    ...
    public void LoadTextDocument(string text, string? sourceLabel = null, string? sourcePath = null, SaveFileKind kind = SaveFileKind.TextDocument)
    {
        ...
        SetCurrentFileReference(sourcePath, kind);
        OnPropertyChanged(nameof(ColumnCount));
        RequestRebuildColumns?.Invoke(this, EventArgs.Empty);
        RefreshStatus();
    ...
    public bool LoadRecoverySnapshot(WorkspaceRecoveryWorkspace workspace)
    {
        ...
        SetCurrentFileReference(workspace.CurrentFilePath, workspace.CurrentFileKind);
